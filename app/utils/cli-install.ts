...
// 'sudo-prompt' is required lazily at runtime because it may not be installed in some environments.
// We attempt to require it only when we need to run an elevated command and handle its absence gracefully.
...
/**
 * Lazily obtain a promisified sudo exec function. Returns null if 'sudo-prompt' cannot be required.
 */
const getSudoExec = ():
...
sudo ln -sf "${cliScriptPath}" "${cliLinkPath}"`,
      ...
      if (result.response === 0) {
        const execFn = getSudoExec();
        if (!execFn) {
          const alt = await dialog.showMessageBox({
            message:
              'Cannot elevate privileges automatically because the "sudo-prompt" module is not available in this environment. You can copy the command and run it manually in a terminal:',
            type: "warning",
          ...
          try {
            await execFn(`ln -sf "${cliScriptPath}" "${cliLinkPath}"`, {
              name: "Hyper",
            });
            return;
          } catch (_error) {
  ...
  return new Promise<void>((resolve, reject) => {
    try {
      ...
      if (pathItem) {
        ...
        const existingOldPath = pathParts.some((pathPart) =>
          pathPart.startsWith(oldPath),
        );
        ...
        if (existingOldPath)
          pathParts = pathParts.filter(
            (pathPart) => !pathPart.startsWith(oldPath),
          );
...
export const installCLI = async (withNotification: boolean) => {
  if (process.platform === "win32") {
    try {
      ...
      logNotify(
        withNotification,
        "Hyper CLI installation failed",
        `Failed to add Hyper CLI path to user PATH ${err}`,
      );
    }
    ...
    try {
      ...
      logNotify(
        withNotification,
        "Hyper CLI installed",
        `Symlink created at ${cliLinkPath}`,
      );
    } catch (error) {
