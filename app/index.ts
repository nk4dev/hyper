  ...
  installDevExtensions(isDev)
    .then(() => {
      function createWindow(
        ...
        const focusedWindow =
          BrowserWindow.getFocusedWindow() || app.getLastFocusedWindow();
      ...
      const makeMenu = () => {
        const menu = plugins.decorateMenu(
          AppMenu.createMenu(createWindow, plugins.getLoadedPluginVersions),
        );
      ...
      if (!isDev) {
        ...
        if (
          config.getConfig().defaultSSHApp &&
          !app.isDefaultProtocolClient("ssh")
        ) {
