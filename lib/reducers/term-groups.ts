...
const insertRebalance = (oldSizes: ImmutableType<number[]>, index: number) => {
  ...
  return [
    ...balanced.slice(0, index).asMutable(),
    newSize,
    ...balanced.slice(index).asMutable(),
  ];
};
...
const splitGroup = (state: ITermState, action: SessionAddAction) => {
  ...
  let parentGroup = activeGroup.parentUid
    ? state.termGroups[activeGroup.parentUid]
    : activeGroup;
  ...
  const newChildren = [
    ...children.slice(0, index).asMutable(),
    newSession.uid,
    ...children.slice(index).asMutable(),
  ];
  state = state.setIn(
    ["termGroups", parentGroup.uid],
    parentGroup.merge({
...
const replaceParent = (
  ...
  if (parent.parentUid) {
    ...
    // with `child`:
    const newChildren = parentParent.children.map((uid: string) =>
      uid === parent.uid ? child.uid : uid,
    );

    state = state.setIn(
      ["termGroups", parentParent.uid, "children"],
      newChildren,
    );
  } else {
...
const removeGroup = (state: ITermState, uid: string) => {
  ...
  if (group.parentUid && state.termGroups[group.parentUid]) {
    ...
    if (newChildren.length === 1) {
      ...
      state = state.setIn(
        ["termGroups", group.parentUid, "children"],
        newChildren,
      );
      if (parent.sizes) {
