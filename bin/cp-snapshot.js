...
function copySnapshot(pathToElectron, archToCopy) {
  ...
  if (fs.existsSync(pathToBlob)) {
    try {
      ...
      console.warn(
        "Failed to copy snapshot_blob.bin:",
        err && err.message ? err.message : err,
      );
    }
  } else {
    console.warn("Snapshot file not found:", pathToBlob);
  }
  ...
  if (fs.existsSync(pathToBlobV8)) {
    try {
      fs.copyFileSync(
        pathToBlobV8,
        path.join(pathToElectron, v8ContextFileName),
      );
      copiedAny = true;
    } catch (err) {
      console.warn(
        "Failed to copy v8 context snapshot:",
        err && err.message ? err.message : err,
      );
    }
  } else {
    console.warn("V8 context snapshot file not found:", pathToBlobV8);
  }

  if (!copiedAny) {
    console.warn(
      "No snapshot files were copied; continuing without snapshots.",
    );
  }
}
